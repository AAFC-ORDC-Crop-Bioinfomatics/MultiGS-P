# omniGS/evaluation/splits.py
import numpy as np
from sklearn.model_selection import KFold
from omniGS.utils.io import write_json
import logging
...
def make_splits(n_samples, n_replicates, n_folds, seed, out_path: str,
    ...
    """
    Generate reproducible CV splits (train/val indices).

    Args:
        n_samples (int): number of samples in dataset
        n_replicates (int): number of CV repetitions
        n_folds (int): number of folds
        seed (int): random seed
        out_path (str): full path where splits.json should be written
        trait (str, optional): phenotype trait name for which splits are generated
        geno_base (str, optional): base name of genotype dataset
		logger (logging.Logger, optional): A logger to use for this function. Defaults to None, in which case a new logger is created.
